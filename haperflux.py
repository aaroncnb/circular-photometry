...
# override columns if /dopol keyword is set
    ...
    for i in range(0, ncalc):  #Be careful about the indexing here (IDL vs. Python style)
        ...
        if (noise_model == 1):
            ...
            fd_err = factor * sqrt(float(ninnerpix) +                 (k * float(ninnerpix)**2/nbgpix)) * mad_std(hmap[bgpix]) 
